...
def key_callback(window, key, scancode, action, mods):
    global g_cam_ang, g_cam_height, g_scale
...
def main():
    ...
    window = glfwCreateWindow(800, 800, '2022056562', None, None)
    if not window:
        glfwTerminate()
        return
    ...
    arm = Node(base, glm.translate(glm.vec3(.2,0,0)), glm.translate((.2,0,.01)) * glm.scale((.5,.1,0.)), glm.vec3(1,0,0))
    finger1 = Node(arm, glm.translate(glm.vec3(.3,.1,0)), glm.translate((.5,0,.01)) * glm.scale((.2,.1,0.)), glm.vec3(0,1,0))
    ...
    while not glfwWindowShouldClose(window):
        ...
        finger1.set_joint_transform(glm.rotate(min(max(glm.cos(t),0.1),0.5), (0,1,0)))
